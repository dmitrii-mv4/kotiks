...
use App\Http\Controllers\Admin\ModuleGenerator\Create\MiddlewareGeneratorController;
use App\Http\Controllers\Admin\ModuleGenerator\Create\PolicyGeneratorController;
...
class CreateModuleController extends Controller
{
    ...
    public function store(ModulesGeneratorCreate $request)
    {
        ...
        $middlewareGenerator = new MiddlewareGeneratorController();
        $policyGenerator = new PolicyGeneratorController();
        ...
        $middlewareGenerator->createMiddleware($validated, $modelName);

        // Генерируем Policy
        $policyGenerator->createPolicies($validated, $modelName);
        ...
        foreach ($permissions as $action => $title) {
            Permission::create([
                'name' => 'module_' . $validated['code'] . '_' . $action,
                'title' => $title,
            ]);
        }
    ...
    // {
    //     $moduleName = Str::studly($module); // Название модуля

    //     // === Удаление разрешений ===

    //     $moduleForPermission = 'module_' . $module;
    //         // Поиск через метод модели
    //         $permissionsToDelete = Permission::findByCodeModule($moduleForPermission);

    //         // Удаление в контроллере
    //         foreach ($permissionsToDelete as $permission) {
    //             $permission->delete();
    //         }
    //     // === end Удаление разрешений ===
    ...
    //         // === Удаление папки Middleware модуля ===
